...
function GameBets({ game, players, teams }) {
  ...
  const team_a_bets = bets.filter(
    bet => bet && (bet.scored() ? bet.team_a_score > bet.team_b_score : bet.winner === game.team_a),
  )
  const team_b_bets = bets.filter(
    bet => bet && (bet.scored() ? bet.team_a_score < bet.team_b_score : bet.winner === game.team_b),
  )
  const draw_bets = bets.filter(bet => bet && bet.team_a_score === bet.team_b_score)
